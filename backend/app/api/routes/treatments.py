...
def can_access(user, patient):
    # Imported dataset encounters are shared read/write records within the application.
    # Role permissions below still control who may create treatment records.
    if patient.dataset_encounter_id is not None:
        return user.role in {"doctor", "hospital_administrator", "healthcare_researcher", "system_administrator"}
    if user.role == "doctor":
...
def create(payload: TreatmentCreate, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.flush()
    record_audit(
        db, user.id, "TREATMENT_CREATE",
        f"Patient {patient.id}: {treatment.treatment_name}, effectiveness={treatment.effectiveness_score}",
    )
    db.commit()
...
def list_(pid: int, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not can_access(user, patient):
        raise HTTPException(403, "Patient is outside your access scope")
    return (db.query(Treatment).filter(Treatment.patient_id == pid).order_by(Treatment.id.desc()).all())
