...
def can_access(user, patient):
    if patient.dataset_encounter_id is not None:
        return user.role in {
            "doctor", "hospital_administrator", "healthcare_researcher",
...
def predict(
    ...
    record_audit(
        ...
        user.id,
        "PREDICTION",
        f"Manual prediction: risk={category}, probability={probability:.4f}, model={version}",
    )
    db.commit()
...
def predict_patient(
    ...
    record_audit(
        ...
        user.id,
        "PREDICTION",
        f"Patient {pid}: risk={category}, probability={probability:.4f}, model={version}",
    )
    db.commit()
