...
import {Activity,BrainCircuit,FileText,LogOut,ShieldCheck,Users,Database,Settings,BarChart3,Stethoscope,HeartPulse,Download,Eye} from "lucide-react";
...
function Layout({user,logout,children}){
 ...
 if(r==="hospital_administrator") nav.push(["/","Hospital Dashboard"],["/patients","Patient Outcomes"],["/prediction","Readmission Forecasts"],["/treatments","Treatment Effectiveness"],["/analysis","Treatment Effectiveness Analysis"],["/hospital-analytics","Hospital Analytics"],["/precautions","Operational Care Support"]);
 if(r==="healthcare_researcher") nav.push(["/","Research Dashboard"],["/patients","Anonymized Dataset"],["/prediction","Risk Prediction"],["/treatments","Treatment Effectiveness (View Only)"],["/analysis","Treatment Analysis"],["/model","AI Model Results"],["/research-export","Research Dataset Export"],["/population","Population Health"]);
 if(r==="system_administrator") nav.push(["/","System Dashboard"],["/patients","Dataset Predictions"],["/prediction","Risk Prediction"],["/treatments","Treatment Records (View Only)"],["/users","User & Role Management"],["/dataset-admin","Dataset Management"],["/settings","System Settings"]);
...
function Treatments({user}){
 const [patients,setPatients]=useState([]),[treatments,setTreatments]=useState([]),[message,setMessage]=useState(""),[error,setError]=useState("");
 const canEdit = user?.role === "doctor" || user?.role === "hospital_administrator";
 ...
 useEffect(()=>{api.get("/patients?limit=100").then(r=>setPatients(r.data)).catch(()=>setError("Unable to load patients"))},[]);
 async function loadTreatments(patientId){
   if(!patientId){setTreatments([]);return;}
   try{setTreatments((await api.get(`/treatments/patient/${Number(patientId)}`)).data)}
   catch(err){setError(err.response?.data?.detail||"Unable to load treatment records")}
 }
 function selectPatient(e){const patient_id=e.target.value;setForm({...form,patient_id});setMessage("");setError("");loadTreatments(patient_id)}
 async function submit(e){
   e.preventDefault();setError("");setMessage("");
   try{
     const saved=(await api.post("/treatments",{...form,patient_id:Number(form.patient_id),recovery_score:Number(form.recovery_score),effectiveness_score:Number(form.effectiveness_score)})).data;
     setMessage("Treatment saved successfully.");
     setTreatments(prev=>[saved,...prev]);
   }catch(err){setError(err.response?.data?.detail||"Unable to save treatment")}
 }
 return <><h1>Treatment Effectiveness</h1>
 <div className="panel">
  <form className="form" onSubmit={submit}>
   <label>Patient<select required value={form.patient_id} onChange={selectPatient}><option value="">Select patient</option>{patients.map(p=><option key={p.id} value={p.id}>{p.full_name} ({p.dataset_encounter_id})</option>)}</select></label>
   <label>Treatment<input required readOnly={!canEdit} value={form.treatment_name} onChange={e=>setForm({...form,treatment_name:e.target.value})}/></label>
   <label>Medication<input readOnly={!canEdit} value={form.medication} onChange={e=>setForm({...form,medication:e.target.value})}/></label>
   <label>Effectiveness 0-100<input type="number" min="0" max="100" readOnly={!canEdit} value={form.effectiveness_score} onChange={e=>setForm({...form,effectiveness_score:e.target.value})}/></label>
   <label>Outcome<select disabled={!canEdit} value={form.outcome} onChange={e=>setForm({...form,outcome:e.target.value})}><option>Ongoing</option><option>Improved</option><option>No Change</option><option>Adverse</option></select></label>
   {user?.role === "doctor" || user?.role === "hospital_administrator" ? <button>Save Treatment</button> : <p><small>View-only access for this role. Treatment records cannot be modified.</small></p>}
  </form>
  {error&&<div className="err">{error}</div>}{message&&<div className="success">{message}</div>}
 </div>
 {form.patient_id && <div className="panel treatment-history"><h2>Saved Treatment Records</h2>
   {treatments.length===0 ? <p>No treatment records found for this patient yet.</p> :
   <div style={{overflowX:"auto"}}><table><thead><tr><th>Date</th><th>Treatment</th><th>Medication</th><th>Effectiveness</th><th>Outcome</th></tr></thead>
   <tbody>{treatments.map(t=><tr key={t.id}><td>{t.start_date}</td><td>{t.treatment_name}</td><td>{t.medication||"—"}</td><td><strong>{Number(t.effectiveness_score).toFixed(0)} / 100</strong></td><td>{t.outcome}</td></tr>)}</tbody></table></div>}
 </div>}
 </>;
}
        ...
        <>
            ...
            <div className="panel">
                ...
                {m && (
                    <>
                        <p>
                            <b>{m.model || "Random Forest"}</b> trained on{" "}
                            {m.dataset_rows?.toLocaleString()} encounters.
                            Target: early readmission (&lt;30 days). Model version: <b>{m.model_version || "deployed"}</b>.
                        </p>
                        <div className="success">Prediction engine is locked. Healthcare Researchers can view results but cannot replace or retrain the deployed model.</div>
...
function AdminPage({type}){
 const [data,setData]=useState(null),[error,setError]=useState("");
 useEffect(()=>{api.get("/admin/dataset-status").then(r=>setData(r.data)).catch(e=>setError(e.response?.data?.detail||"Unable to load"))},[]);
...
export default function App(){
 ...
 return <Layout user={user} logout={logout}><Routes>
  ...
  <Route path="/treatments" element={<Protected user={user} allowed={["doctor","hospital_administrator","healthcare_researcher","system_administrator"]}><Treatments user={user}/></Protected>}/>
  ...
  <Route path="/model" element={<Protected user={user} allowed={["healthcare_researcher","system_administrator","doctor","hospital_administrator"]}><ModelMetrics user={user}/></Protected>}/>
